                        ...
                        @if ($blogs->isNotEmpty())
                            @foreach ($blogs as $key => $data)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>
                                        <img src="{{ asset($data->image->url) }}" alt="Blog Image" width="100px">
                                    </td>
                                    <td>{{ $data->title ?? '' }}</td>
                                    <td>{{ $data->category ?? '' }}</td>
                                    <td>
                                        <div class="form-check form-switch">
                                            <input class="form-check-input Blog-status-toggle" type="checkbox"
                                                id="flexSwitchCheckDefault{{ $data->id }}"
                                                data-Blog-id="{{ $data->slug }}" {{ $data->status ? 'checked' : '' }}>
                                        </div>
                                    </td>
                                            ...
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        <form action="{{ route('blog.destroy', $data->slug) }}"
...
@section('js')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const toggles = document.querySelectorAll('.Blog-status-toggle');
